...
import {
  ...
  Clock, AlertCircle, RotateCcw, Loader2, Plus,
  ChevronRight,
} from 'lucide-react';
...
export default function PodDetailPage() {
  ...
  async function handleTransition(taskId: string, toStatus: TaskStatus) {
    ...
    loadBoard(selectedSprintId || undefined);
  }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {pod.description && (
              <p className="text-muted-foreground text-sm mt-0.5">{pod.description}</p>
            )}
          </div>
        </div>
      ...
      {board?.sprint && (
        <div className="flex items-center gap-3 text-xs text-muted-foreground bg-card border rounded-lg px-4 py-2.5">
          ...
          <Badge variant={board.sprint.status === 'active' ? 'default' : 'secondary'} className="text-[10px]">
            {board.sprint.status}
          </Badge>
          ...
          <Link href={`/sprints/${board.sprint.id}`}>
            <Button variant="ghost" size="sm" className="h-6 text-xs gap-1 px-2">
              View Sprint <ChevronRight className="w-3 h-3" />
            </Button>
          </Link>
        </div>
      )}
        ...
        <div className="flex items-center gap-3 text-xs text-muted-foreground bg-muted/30 border rounded-lg px-4 py-2.5">
          ...
          {isManager && (
            <Link href="/sprints/new">
              <Button variant="outline" size="sm" className="h-6 text-xs ml-2">Create Sprint</Button>
            </Link>
          )}
        </div>
      )}
...
function KanbanColumn({
  ...
  return (
    <div className={`flex-none w-[280px] bg-card border rounded-xl flex flex-col border-t-2 ${borderColor}`}>
      {/* Column header */}
      ...
      <div className="flex-1 p-2 space-y-2 overflow-y-auto max-h-[calc(100vh-320px)]">
        {column.tasks.length === 0 ? (
          <div className="py-6 text-center text-[11px] text-muted-foreground opacity-60">Empty</div>
        ) : (
...
function KanbanCard({
  ...
  const available = canTransition
    ? (ALLOWED_TRANSITIONS[task.status as TaskStatus] ?? [])
    : [];

  return (
    <Link href={`/tasks/${task.id}`}>
      <div
        className="bg-background border rounded-lg p-3 space-y-2.5 hover:border-primary/40 transition-colors cursor-pointer group"
        ...
        <div className="flex items-center justify-between gap-1">
          ...
          {task.priority >= 1 && (
            <span className={`text-[10px] font-medium shrink-0 ${task.priority >= 2 ? 'text-destructive' : 'text-orange-400'}`}>
              {task.priority >= 2 ? '●●' : '●'} {task.priority >= 2 ? 'Critical' : 'High'}
            </span>
          )}
        </div>
